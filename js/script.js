...
function turnDark() {
  ...
  button = document.getElementById("dark-button");
  const prefixTexts = document.getElementsByClassName("prefix");
  if (button.innerText === "DARK MODE") {
    button.innerText = "LIGHT MODE";
    button.classList.remove("btn-dark")
    button.classList.add("btn-light")
    for (let i = 0; i < prefixTexts.length; i++) {
      prefixTexts[i].style.color = "white";
    }
  } else {
    ...
    for (let i = 0; i < prefixTexts.length; i++) {
        prefixTexts[i].style.color = "#4f4f4f";
      }
  }
}
